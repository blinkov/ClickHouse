...
Returns an array of numbers from start to end-1 by step. If the argument `start` is not specified, defaults to 0. If the argument `step` is not specified, defaults to 1. It behaviors almost like pythonic `range`. But the difference is that all the arguments type must be `UInt` numbers. Just in case, an exception is thrown if arrays with a total length of more than 100,000,000 elements are created in a data block.
...
-   `arrays` – Arbitrary number of arguments of [Array](../../sql-reference/data-types/array.md) type. **Example**
...
!!! note "Note" To improve sorting efficiency, the [Schwartzian transform](https://en.wikipedia.org/wiki/Schwartzian_transform) is used.
...
**Returned value** Returns AUC value with type Float64.
